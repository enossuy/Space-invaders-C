...
#define LEFT -1
#define STATIC 0
#define MISSILES_LIMIT 700
#define INVADERS_INIT_DX 50
#define INVADERS_INIT_DY 200
#define GAP 60

// Initializes id sprites
void init_sprites(int *shield_img, int *ship_img, int *ship_killed_img,
                  int *invader_killed_img, int *invader_1_img_1,
                  int *invader_2_img_1, int *invader_3_img_1, int *missile_img,
                  int *bomb_img) {
...
entity_list_t *init_shields(int shield_img, int width) {
  ...
  int even_space = space_left / (SHIELDS + 1); // 4 gaps left in total  
  for (int i = 0; i < SHIELDS; i++) {
    push_to_head(shields_list,
                 even_space + i * even_space + ((i * width) - (i % 1) * width), // this formula places the shields perfectly
                 SHIELD_Y, shield_img, STATIC);
  }
  return shields_list;
}

// returns a list of invaders 
entity_list_t *init_invaders(int invader_1_img_1, int invader_2_img_1,
                             int invader_3_img_1, int direction) {
  entity_list_t *invaders_list = createLinkedList();

  for (int i = 0; i < INVADERS_PER_ROW; i++) {
    push_to_head(invaders_list, i * INVADERS_INIT_DX, INVADERS_INIT_DY, invader_1_img_1, direction);    // Initializing invaders coordinates
    push_to_head(invaders_list, i * INVADERS_INIT_DX, INVADERS_INIT_DY + GAP, invader_2_img_1, direction);
    push_to_head(invaders_list, i * INVADERS_INIT_DX, INVADERS_INIT_DY + 2*GAP, invader_3_img_1, direction);
  }
  return invaders_list;
}

// to display all entities in a given entity list
void display_entity(entity_list_t list) {
  Node_t *element = list.head;
  while (element != NULL) {
    afficherLutin(element->entity.entity_id, element->entity.x_coordinates,
                  element->entity.y_coordinates);
    element = element->next_entity;
  }
}

// moves invaders down and changes each invader's direction 
...
// loops trough the invaders_list and tests if any invader is at the edge, if so moves down , if not moves them horizontally

void move_invaders(entity_list_t *invaders_list) {
  Node_t *invader = invaders_list->head;
  bool edge_test = false;
  while (invader != NULL) {
    if (invader->entity.x_coordinates - INVADERS_DX <= 0 &&
        invader->entity.direction == -1) {
      edge_test = true;
      break;
    }
    if (invader->entity.x_coordinates + INVADERS_DX >= SCREEN_WIDTH - 35 &&
        invader->entity.direction == 1) {
      edge_test = true;
      break;
    }
...
// manages missiles initialization
void init_missiles(int missile_img, int ship_x, int ship_y,
                   entity_list_t *missiles, char key, evenement event) {
  if (key == 'r' && event == toucheBas) {
    if (missiles->head != NULL) {
      if (missiles->head->entity.y_coordinates < MISSILES_LIMIT)
        push_to_head(missiles, ship_x, ship_y, missile_img, STATIC);
    } else
      push_to_head(missiles, ship_x, ship_y, missile_img, STATIC);
  }
}

//manages missiles movement
void move_missiles(entity_list_t *missiles) {

  Node_t *missile = missiles->head;
  while (missile != NULL) {
    afficherLutin(missile->entity.entity_id, missile->entity.x_coordinates,
                  missile->entity.y_coordinates);
...
// initilaizes dimensions of  a given entity
...
bool hitbox_test(int x1, int y1, int height1, int width1, int x2, int y2,
  ...
  if (((((x1 <= x2_2 && x1 >= x2) || // second entity coming from the left
         (x1_2 <= x2_2 && x1_2 >= x2)) &&
        ((y1 <= y2_2 && y1 >= y2) || // second entity coming from the right
         (y1_2 <= y2_2 && y1_2 >= y2)))))
...
entity_list_t *init_bombs(int bomb_img, entity_list_t *invaders, int width,
                          int height, int random_number) {
  ...
  while (invader != NULL) {
    if (random_number == (rand() % 20)) {
      ...
      int x = invader->entity.x_coordinates + width / 2;
      int y = invader->entity.y_coordinates +
              height; // bombs appear just below the invader

      // Create a new bomb entity with the specified image and coordinates
      entity_t bomb = {
          .entity_id = bomb_img, .x_coordinates = x, .y_coordinates = y};

      // Add the new bomb entity to the bombs list
      push_to_head(bombs_list, bomb.x_coordinates, bomb.y_coordinates,
                   bomb.entity_id, 0);
    }
...
void move_bombs(entity_list_t *bombs) {
  ...
  while (bomb != NULL) {
    afficherLutin(bomb->entity.entity_id, bomb->entity.x_coordinates,
                  bomb->entity.y_coordinates);
...
void collision(entity_list_t *list_1, entity_list_t *list_2) {
  ...
  while (current_node_1 != NULL) {
    ...
    while (current_node_2 != NULL) {
      if (hitbox_test(
              current_node_1->entity.x_coordinates,
              current_node_1->entity.y_coordinates,
              current_node_1->entity.height, 
              current_node_1->entity.width,
              current_node_2->entity.x_coordinates,
              current_node_2->entity.y_coordinates,
              current_node_2->entity.height,
              current_node_2->entity.width)) {
        // If the entities have collided, remove them from their respective
        // lists
        pop_entity(list_1, current_node_1->entity.x_coordinates,
                   current_node_1->entity.y_coordinates);

        pop_entity(list_2, current_node_2->entity.x_coordinates,
                   current_node_2->entity.y_coordinates);
        ...
        // Exit the inner loop since we have already found a collision for this
        // entity
      }
...
int main() {
  // Initialize game screen and window
  ...
  int shield_img = 0;
  int ship_img = 0;
  int ship_killed_img = 0;
  int invader_killed_img = 0;
  int invader_1_img_1 = 0;
  int invader_2_img_1 = 0;
  int invader_3_img_1 = 0;
  int missile_img = 0;
  int bomb_img = 0;
  int direction = RIGHT;
  ...
  while (1) {
    ...
    init_missiles(missile_img, ship.x_coordinates + MISSILE_STARTING_POSITION,
                  ship.y_coordinates, missiles, key, event);
